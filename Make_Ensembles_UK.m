...
Nr_ensembles = length(Parameters.Ensemble_Names);
...
elseif Ensemble == 7
    ...
    % average calculated weights among all options and restrict
    % calculate average deviance among options and eveness (seperate function)
...
elseif Ensemble == 9 || Ensemble == 10
    ...
    Store.EqWeight(:,1) = Store.Weights(:,1);
    Deltas =  Weighting.RegressAmong(:,3);
    nrM = size(model_values_store,2);
     Weights = TakeBetaWeights(Parameters,nrM);
    %Step 2
    %Baseline
    ...
    while its < Parameters.max_its
        ...
        if (ceil(its/5) == (its/5))
             Cur.Weights = TakeBetaWeights(Parameters,nrM);
        end
        for i = 1:1:nrM
            Nwght = Cur.Weights;
    ...
    Store.DisWeights = dataset((nanmean(Store.EqWeight,2)),'Varnames',{'MeanWeight'});
    Store.DisWeights.StdWeight(:,1) = (nanstd(Store.EqWeight,0,2));
    for i = 1:nrM
        ZeroCount(i) = (length(Store.EqWeight(i,(Store.EqWeight(i,:)==0))))./size(Store.EqWeight,2);
    end
    Store.DisWeights.Zerperc(:,1) = ZeroCount;
...
function  [wght] = TakeBetaWeights(Parameters,nrM)
BetarRnd= abs(randn);
wght(:,1) = ShapeWeights((random('beta',BetarRnd,BetarRnd,nrM,1)),Parameters);
end
...
% Clean dataset and replace missing values by means per row for
% calculations
num = [];
...
for j = 1:nrM
    ...
    % clear Outputs take_array InvModels Predicted testArray model values wght
end
if exist('NewRhos','var') == 1
    NewDev = (round(nanmean(NewDevs).*Parameters.Precision(2)))./Parameters.Precision(2);
    NewRho = (round(nanmean(NewRhos).*Parameters.Precision(2)))./Parameters.Precision(2);
else
    NewDev =-9999;
    NewRho = -9999;
end
...
elseif Crit == DecCrit
    ...
    if (Dice>0.5)
        ...
        Store.Rho(improv+2) = NewRho;
        Store.Weights(:,improv+2) =  Cur.Weights;
        its = its; %#ok<ASGSL> %Note no change in iteration sto avoid endless loops
        improv = improv + 1;
        equal = 1;
    end
end
